...
def train(model, cfg, tokenizer, data, batch_size=32, lr=0.001):
    # tokenize training data
    token_ids = torch.tensor(tokenizer.encode(data))
    # create optimization object using Adam algorithm
    optimizer = torch.optim.Adam(model.parameters(), lr=lr)
    # list to append losses to, will be returned for creating loss plots
    training_losses = []
    ...
    for epoch in range(cfg.n_epoch):
        ...
        for i in range(0, len(token_ids) - cfg.n_context * batch_size, cfg.n_context * batch_size):
            x_list, y_list = [], []
            # loop thorugh each set of contexts
            for j in range(0, cfg.n_context * batch_size, cfg.n_context):
                x_list.append(token_ids[i+j:i+j+cfg.n_context])
                y_list.append(token_ids[i+j+1:i+j+cfg.n_context+1])

            # combine tokens into matrices
            ...
            # calculate loss at given step from forward model
            loss = F.cross_entropy(distribution.reshape(-1, cfg.d_vocab), Y.reshape(-1))
            # compute gradients
            loss.backward()
            # ensures model remains stable, gradients don't blow up
            torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=1.0)
            # updates model
            optimizer.step()

            # adds calculate loss to running total
            loss_sum += loss.item()

            # batch counter
            batch += 1
            if batch % 50 == 0:
                print(f'Epoch {epoch+1}/{cfg.n_epoch} | Batch {batch} | Loss: {loss.item():.4f}')

        # calculates loss for each epoch, adds these to list for plotting later
        loss_mean = loss_sum / batch
        training_losses.append(loss_mean)

        
        print(f'Training epoch {epoch+1} / {cfg.n_epoch} completed')
